...
export function check(errorText: string): Validator<"check"> {
  const name: StandardValidatorNameFn<"check"> = (params: { key: string }) =>
    `${params.key}-check`;

  const fn =
    (params: { key: string }) =>
    (control: AbstractControl<string | boolean>) => {
...
export function confirm(
  ...
  const name: ConfirmValidatorNameFn = (params: { key: string }) =>
    `${params.key}-confirm`;

  const fn =
    (params: { key: string; confirmKey: string }) =>
    (control: AbstractControl<string>) => {
      const customName: ConfirmValidatorName = `${params.key}-confirm`;

      return control.get(params.key)?.value !==
        control.get(params.confirmKey)?.value
        ? { [customName]: true }
        : null;
    };
...
export function maxLength(
  ...
  const name: StandardValidatorNameFn<"maxLength"> = (params: {
    key: string;
...
export function min(min: number, errorText: string): Validator<"min"> {
  const name: StandardValidatorNameFn<"min"> = (params: { key: string }) =>
    `${params.key}-min`;
...
export function max(max: number, errorText: string): Validator<"max"> {
  const name: StandardValidatorNameFn<"max"> = (params: { key: string }) =>
    `${params.key}-max`;
...
export function strongPassword(errorText: string): Validator<"strongPassword"> {
  const name: StandardValidatorNameFn<"strongPassword"> = (params: {
    key: string;
...
export function email(errorText: string): Validator<"email"> {
  const name: StandardValidatorNameFn<"email"> = (params: { key: string }) =>
    `${params.key}-email`;
...
export function phone(errorText: string): Validator<"phone"> {
  const name: StandardValidatorNameFn<"phone"> = (params: { key: string }) =>
    `${params.key}-phone`;
